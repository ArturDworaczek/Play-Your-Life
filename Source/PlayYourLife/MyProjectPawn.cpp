...
const FName AMyProjectPawn::EngineAudioRPM("RPM");
...
AMyProjectPawn::AMyProjectPawn()
{
	...
	Vehicle4W->TransmissionSetup.GearSwitchTime = 0.08f;
	...
	// Setup the audio component and allocate it a sound cue
	EngineStartSoundComponent = CreateDefaultSubobject<UAudioComponent>(TEXT("Start Engine Sound"));
	EngineStartSoundComponent->SetSound(EngineStartSound);
	EngineStartSoundComponent->SetupAttachment(GetMesh());
...
void AMyProjectPawn::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("EngineStart", IE_Pressed, this, &AMyProjectPawn::StartEngine);
}
void AMyProjectPawn::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	// Replicate to everyone
	DOREPLIFETIME(AMyProjectPawn, bEngineOn);
}
// ENGINE START
void AMyProjectPawn::StartEngine_Implementation()
{
	if (bEngineOn)
	{
		bEngineOn = false;
...
bool AMyProjectPawn::StartEngine_Validate()
{
	return true;
}
void AMyProjectPawn::StartEngineServer_Implementation()
{
	EngineStartSoundComponent->Play();
	GetWorld()->GetTimerManager().SetTimer(EngineStartTimer, this, &AMyProjectPawn::Start, EngineStartSoundComponent->Sound->Duration, false);
}
void AMyProjectPawn::Start()
{
	EngineStartSoundComponent->Stop();
...
void AMyProjectPawn::EnableIncarView(const bool bState)
{
...
// TICK AND BEGIN PLAY (REMOVE ME PLS)
...
void AMyProjectPawn::SetupInCarHUD()
{
...
// PHYS MATERIAL
void AMyProjectPawn::UpdatePhysicsMaterial()
{
